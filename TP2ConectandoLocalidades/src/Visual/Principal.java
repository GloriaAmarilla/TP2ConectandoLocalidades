...
public class Principal {
	...
	private void initialize() {
		...
		JLabel indicaIngreso = new JLabel(
				"<html><center>Ingrese las localidades<br>que desee conectar</center></html>");
	...
	private void cargarDatosGuardados() {
		for (Localidad l : grafoInicial.getLocalidades()) {
			marcarEnElMapa(l.getLatitud(), l.getLongitud(), l.getProvincia() + ", " + l.getNombre());
		}
		for (Arista a : grafoInicial.getAristas()) {
			dibujarArista(a);
		}
	}

	private void dibujarArista(Arista arista) {
		Coordinate coordOrigen = new Coordinate(arista.getOrigen().getLatitud(), arista.getOrigen().getLongitud());
		Coordinate coordDestino = new Coordinate(arista.getDestino().getLatitud(), arista.getDestino().getLongitud());
